classdef ZS_Contact_s07 < dynamicprops

    methods

    function self = get_s07(self,s07,LINK_OBJ,OPTS)
    %-------------------------------------------------------------------------------
    % Name:           get_s07 [OK]
    % Purpose:        Parse the results from the .s07 file and return it
    % Last Update:    10.10.2023
    %-------------------------------------------------------------------------------

    % Get the total number of GP
    N_GP = LINK_OBJ.MODEL_dat.ELEMENTS.get_total_number_of_GP_group('CONTACTS');

    % Get the list of GP per element
    GP_LST = LINK_OBJ.MODEL_dat.ELEMENTS.get_GP_group('CONTACTS');

    % Get the total number of results from RCF
    N_RES = LINK_OBJ.RESULTS_CONFIG_rcf.ELEMENTS.CONTACT.N_RES;
    
    % Shape the 3D matrix of the results
    contact_res = reshape(s07, N_RES, N_GP, []);
    ...
    BOOLE_EL = LINK_OBJ.MODEL_dat.ELEMENTS.get_element_ID_selection('Contact',OPTS);
    ...
    field_names = fieldnames(LINK_OBJ.RESULTS_CONFIG_rcf.ELEMENTS.CONTACT);
    field_names = {field_names{3:end}}';
    
    count = 1;
    
    for i = 1:LINK_OBJ.RESULTS_CONFIG_rcf.ELEMENTS.CONTACT.N
        ...
        elseif strcmp(field_names{i},"STRESSES")
            ...
            for j = 1:LINK_OBJ.RESULTS_CONFIG_rcf.ELEMENTS.CONTACT.STRESSES.N
                temp = contact_res(count,:,:);
                temp = permute(temp,[2,3,1]);
                temp = mat2cell(temp,GP_LST,ones(1,N_HIS));
                temp = temp(BOOLE_EL,BOOLE_HIS);
                self.STRESSES.(sprintf(strcat('SIG_',LINK_OBJ.RESULTS_CONFIG_rcf.ELEMENTS.CONTACT.STRESSES.Axes(j)))) = temp;
                count = count + 1;
            end
    

        elseif strcmp(field_names{i},"T_STRESSES")

        DYN.T_STRESSES = self.addprop('T_STRESSES');
            
            for j = 1:LINK_OBJ.RESULTS_CONFIG_rcf.ELEMENTS.CONTACT.T_STRESSES.N
                temp = contact_res(count,:,:);
                temp = permute(temp,[2,3,1]);
                temp = mat2cell(temp,GP_LST,ones(1,N_HIS));
                temp = temp(BOOLE_EL,BOOLE_HIS);
                self.T_STRESSES.(sprintf(strcat('T_SIG_',LINK_OBJ.RESULTS_CONFIG_rcf.ELEMENTS.CONTACT.T_STRESSES.Axes(j)))) = temp;
                count = count + 1;
            end
        ...
        elseif strcmp(field_names{i},"STRAINS")
            ...
            for j = 1:LINK_OBJ.RESULTS_CONFIG_rcf.ELEMENTS.CONTACT.STRAINS.N
                temp = contact_res(count,:,:);
                temp = permute(temp,[2,3,1]);
                temp = mat2cell(temp,GP_LST,ones(1,N_HIS));
                temp = temp(BOOLE_EL,BOOLE_HIS);
                self.STRAINS.(sprintf(strcat('EPS_',LINK_OBJ.RESULTS_CONFIG_rcf.ELEMENTS.CONTACT.STRAINS.Axes(j)))) = temp;
                count = count + 1;
            end
    ...
    if ~BOOLE_QoI
    QoI = OPTS.QoI;
    All_QoI = LINK_OBJ.RESULTS.get_class_QoI("CONTACTS");
